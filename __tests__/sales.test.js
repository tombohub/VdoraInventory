...
test('to be cookie jar', async () => {
    const cookieJar = await sales.getCookies()
    const salesData = await sales.getSalesData(ordersUrl, cookieJar)
     expect(cookieJar).toBeDefined()
     expect(salesData.draw).toBeTruthy()
})
